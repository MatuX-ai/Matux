...
class LearningSource(Base):
    ...
    org_id = Column(Integer, ForeignKey("organizations.id",
                    use_alter=True), nullable=True, index=True)

    # 学习来源类型
    source_type = Column(Enum(LearningSourceType), nullable=False, index=True)

    # 来源状态
    status = Column(Enum(LearningSourceStatus),
                    default=LearningSourceStatus.ACTIVE, index=True)
    ...
    organization = relationship(
        "Organization", back_populates="learning_sources")
    enrollments = relationship(
        "CourseEnrollment", back_populates="learning_source")
    unified_records = relationship(
        "UnifiedLearningRecord", back_populates="learning_source")
...
OrgModel.learning_sources = relationship(
    "LearningSource",
...
class LearningSourceCreate(BaseModel):
    ...
    source_detail: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="来源详情")
...
class LearningSourceUpdate(BaseModel):
    ...
    source_type: Optional[LearningSourceType] = Field(
        None, description="学习来源类型")
    status: Optional[LearningSourceStatus] = Field(None, description="来源状态")
    name: Optional[str] = Field(
        None, min_length=1, max_length=255, description="来源名称")
