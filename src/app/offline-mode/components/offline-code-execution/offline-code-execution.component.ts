...
@Component({
  ...
  template: `
    <mat-card class="code-exec-card">
      <mat-card-header>
        ...
        <mat-card-subtitle>{{ executionService.getExecutionStatusDescription() }}</mat-card-subtitle>
      </mat-card-header>
      ...
      <mat-card-content>
        ...
        <div class="language-selector">
          <mat-select [(ngModel)]="selectedLanguage" class="lang-select">
            <mat-select-trigger>
              <mat-icon>{{ selectedLanguage === 'python' ? 'data_object' : 'javascript' }}</mat-icon>
        ...
        <!-- 执行进度 -->
        <mat-progress-bar
          *ngIf="executionService.executing$ | async"
          mode="indeterminate">
        </mat-progress-bar>

        <!-- 执行结果 -->
        <div *ngIf="lastResult" class="result-panel"
        ...
        <div *ngIf="(executionService.history$ | async)?.length" class="history-section">
          <h4>执行历史</h4>
          <mat-chip-listbox>
            <mat-chip *ngFor="let entry of executionService.history$ | async | slice:0:5"
  ...
  `,
  styles: [`
    .code-exec-card { margin: 16px; }
    .language-selector { display: flex; align-items: center; gap: 12px; margin-bottom: 12px; }
    .lang-select { width: 180px; }
    .code-editor-wrapper { margin-bottom: 12px; }
    .code-textarea {
      width: 100%; min-height: 200px; padding: 12px;
      font-family: 'Geist Mono', 'Consolas', monospace; font-size: 14px;
      background: #0f172a; color: #22c55e; border: 1px solid #334155;
      border-radius: 12px; resize: vertical; line-height: 1.5;
    }
    .result-panel { margin-top: 12px; padding: 12px; border-radius: 12px; }
    ...
    .execution-time { margin-left: auto; color: #64748b; font-size: 12px; }
    .result-output, .result-error {
      margin: 0; padding: 8px; font-family: monospace; font-size: 13px;
      white-space: pre-wrap; word-break: break-word;
    }
    .result-output { color: #166534; }
    .result-error { color: #dc2626; }
    .history-section { margin-top: 16px; }
    .history-section h4 { margin-bottom: 8px; color: #475569; }
  `],
})
export class OfflineCodeExecutionComponent implements OnInit {
  ...
  ngOnInit(): void {
    // 设置默认代码
    this.code = this.selectedLanguage === 'python'
      ? '# 在此输入 Python 代码\nprint("Hello, MatuX!")\n'
