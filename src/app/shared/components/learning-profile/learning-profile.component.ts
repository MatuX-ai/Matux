...
import { AbilityDimensions, StudentLearningProfile } from '../../../core/models/ai-teacher.models';
...
@Component({
        ...
        <mat-card-content>
          <div class="radar-chart-placeholder">
            <svg viewBox="0 0 300 300" class="radar-svg">
              <!-- 绘制雷达图背景 -->
              <polygon
                *ngFor="let level of radarLevels"
              ...
              <!-- 绘制数据区域 -->
              <polygon
                [attr.points]="getAbilityRadarPoints()"
              ...
              <!-- 绘制数据点标签 -->
              <text
                *ngFor="let dim of abilityDimensionList; let i = index"
                [attr.x]="getLabelX(i)"
                [attr.y]="getLabelY(i)"
                text-anchor="middle"
                font-size="12"
  ...
  styles: [
    `
      ...
      .mastered .skill-status-icon {
        color: #22c55e;
      }
      .learning .skill-status-icon {
        color: #f59e0b;
      }
      .locked .skill-status-icon {
        color: #94a3b8;
      }
      .skill-name {
        color: #334155;
...
export class LearningProfileComponent implements OnChanges {
  ...
  getAbilityRadarPoints(): string {
    ...
    const cx = 150,
      cy = 150,
      maxR = 120;
    return this.abilityDimensionList
      .map((dim, i) => {
        const angle = (Math.PI * 2 * i) / n - Math.PI / 2;
