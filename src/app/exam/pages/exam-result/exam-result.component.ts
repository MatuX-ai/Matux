...
@Component({
  ...
  template: `
      ...
      <div class="result-card" [class.passed]="passed" [class.failed]="!passed">
        ...
        <div class="stats-grid">
          ...
          <div class="stat-item">
            ...
            <span class="stat-value">{{ (attempt.submitted_at || attempt.started_at) | date:'short' }}</span>
          </div>
        </div>
    ...
    <div *ngIf="error" class="error-state"><p>{{ errorMessage }}</p><button (click)="loadResult()">重试</button></div>
  `,
  styles: [
    `
      .result-container { max-width: 600px; margin: 0 auto; padding: 32px 16px; }
      .result-card { background: white; border-radius: 16px; padding: 40px; text-align: center; box-shadow: 0 4px 24px rgba(0,0,0,0.1); }
      .result-card.passed { border-top: 4px solid #34c759; }
      .result-card.failed { border-top: 4px solid #ff3b30; }
      ...
      .stat-label { display: block; font-size: 12px; color: #86868b; margin-bottom: 4px; }
...
export class ExamResultComponent implements OnInit {
  ...
  loadResult(): void {
    ...
    this.examService.getAttempt(attemptId).pipe(
      catchError((err) => {
        this.error = true;
  ...
  statusLabel(s: string): string {
    const labels: Record<string, string> = { in_progress: '进行中', submitted: '待评分', graded: '已评分', timed_out: '超时', voided: '作废' };
    return labels[s] || s;
  }
}
