...
export class VectorKnowledgeService {
  ...
  getKnowledgeForStage(stage: LearningStage): Observable<KnowledgeSearchResult[]> {
    return this.http.get<KnowledgeSearchResult[]>(`${this.API_BASE}/stage/${stage}`).pipe(
      catchError(() => of(this.getStageKnowledge(stage))),
    );
  }
  ...
  private readonly GLOBAL_KNOWLEDGE: KnowledgeSearchResult[] = [
    {
      item_id: 'g_python_variables', content: '变量与数据类型：Python 中的 int、float、str、bool 四种基本类型，变量命名规则（字母/下划线开头，区分大小写）',
      category: 'programming', layer: 'global', difficulty: 'beginner',
      tags: ['python', '变量', '数据类型'], metadata: {}, relevance_score: 0.95,
    },
    {
      item_id: 'g_python_conditions', content: '条件判断：if/elif/else 语法，比较运算符（==, !=, <, >, <=, >=），逻辑运算符（and, or, not）',
      category: 'programming', layer: 'global', difficulty: 'beginner',
      tags: ['python', '条件判断'], metadata: {}, relevance_score: 0.92,
    },
    {
      item_id: 'g_python_loops', content: '循环结构：for 循环（for item in iterable）、while 循环、range() 函数、break/continue 控制流、循环嵌套',
      category: 'programming', layer: 'global', difficulty: 'intermediate',
      tags: ['python', '循环', 'for', 'while'], metadata: {}, relevance_score: 0.88,
    },
    {
      item_id: 'g_python_functions', content: '函数定义与调用：def 关键字、参数（位置/默认/关键字/*args/**kwargs）、返回值、作用域规则（LEGB）',
      category: 'programming', layer: 'global', difficulty: 'intermediate',
      tags: ['python', '函数', '作用域'], metadata: {}, relevance_score: 0.85,
    },
    {
      item_id: 'g_python_lists', content: '列表与元组：list 创建/索引/切片/方法（append, remove, sort）、tuple 不可变性、列表推导式',
      category: 'programming', layer: 'global', difficulty: 'beginner',
      tags: ['python', '列表', '元组', '数据结构'], metadata: {}, relevance_score: 0.90,
    },
    {
      item_id: 'g_stem_circuit', content: '电路基础：电压(V)、电流(I)、电阻(R) 的欧姆定律 V=IR，串联/并联电路特点，LED、电阻、开关的基本用法',
      category: 'stem', layer: 'global', difficulty: 'beginner',
      tags: ['stem', '电路', '欧姆定律'], metadata: {}, relevance_score: 0.82,
    },
    {
      item_id: 'g_stem_sensors', content: '传感器原理：光敏电阻、温度传感器(DHT11)、超声波测距(HC-SR04)、红外传感器的工作原理与Arduino接线方式',
      category: 'stem', layer: 'global', difficulty: 'intermediate',
      tags: ['stem', '传感器', 'Arduino'], metadata: {}, relevance_score: 0.78,
    },
    {
      item_id: 'g_methodology_debug', content: '调试方法论：print 调试法、二分定位法、阅读错误栈（Traceback）、单元测试基础、代码审查',
      category: 'methodology', layer: 'global', difficulty: 'intermediate',
      tags: ['方法论', '调试', '错误处理'], metadata: {}, relevance_score: 0.75,
    },
    {
      item_id: 'g_ai_ml_basics', content: 'AI 基础知识：机器学习三要素（数据、模型、算法），监督学习/无监督学习/强化学习区分，训练/验证/测试流程',
      category: 'ai', layer: 'global', difficulty: 'advanced',
      tags: ['AI', '机器学习', '深度学习'], metadata: {}, relevance_score: 0.70,
    },
  ];
  ...
  private getStageKnowledge(stage?: LearningStage): KnowledgeSearchResult[] {
    const stageBase: KnowledgeSearchResult[] = [
      {
        item_id: 's_l1_blockly', content: 'L1 Blockly 编程入门：积木块拼接、顺序执行、循环积木、条件积木、事件驱动',
        category: 'programming', layer: 'stage', difficulty: 'beginner',
        tags: ['blockly', '编程入门', '可视化'], metadata: { stage: 'L1' }, relevance_score: 0.90,
      },
      {
        item_id: 's_l2_python_transition', content: 'L2 Blockly → Python 过渡：将积木逻辑翻译为 Python 代码，认识缩进、关键字、基础语法',
        category: 'programming', layer: 'stage', difficulty: 'beginner',
        tags: ['blockly', 'python', '过渡'], metadata: { stage: 'L2' }, relevance_score: 0.88,
      },
      {
        item_id: 's_l3_python_advanced', content: 'L3 Python 进阶：文件读写、异常处理、模块导入、函数式编程（map/filter/lambda）、面向对象入门',
        category: 'programming', layer: 'stage', difficulty: 'intermediate',
        tags: ['python', '进阶'], metadata: { stage: 'L3' }, relevance_score: 0.85,
      },
      {
        item_id: 's_l3_data_structures', content: 'L3 数据结构：栈(Stack)、队列(Queue)、链表(LinkedList) 的概念与 Python 实现，时间/空间复杂度入门',
        category: 'programming', layer: 'stage', difficulty: 'intermediate',
        tags: ['数据结构', '算法'], metadata: { stage: 'L3' }, relevance_score: 0.82,
      },
      {
        item_id: 's_l4_algorithm', content: 'L4 算法基础：排序（冒泡/快排/归并）、搜索（二分/DFS/BFS）、动态规划入门、贪心算法',
        category: 'programming', layer: 'stage', difficulty: 'advanced',
        tags: ['算法', '竞赛'], metadata: { stage: 'L4' }, relevance_score: 0.80,
      },
      {
        item_id: 's_l4_ai_project', content: 'L4 AI 实战项目：使用 Python 实现简单的图像分类、文本情感分析、聊天机器人',
        category: 'ai', layer: 'stage', difficulty: 'advanced',
        tags: ['AI', '项目实战'], metadata: { stage: 'L4' }, relevance_score: 0.78,
      },
    ];
    ...
    return [
      {
        ...
        category: 'programming', layer: 'personal', difficulty: 'beginner',
        tags: ['range', '循环', '个性化'], metadata: { userId }, relevance_score: 0.95,
      },
      {
        item_id: `p_${userId}_loop_mistake`,
        content: `[个性化] 该生循环常见错误：忘记缩进循环体、range() 参数顺序混淆、for/while 使用场景不分、循环变量未更新导致死循环`,
        category: 'programming', layer: 'personal', difficulty: 'intermediate',
        tags: ['循环', '常见错误', '个性化'], metadata: { userId }, relevance_score: 0.92,
      },
      {
        item_id: `p_${userId}_blockly_transition`,
        content: `[个性化] 该生的 Blockly → Python 过渡记录：已完成 Blockly 循环关卡 80%，推荐的对照练习是"积木转代码"模式`,
        category: 'programming', layer: 'personal', difficulty: 'beginner',
        tags: ['blockly', '转换', '个性化'], metadata: { userId }, relevance_score: 0.88,
      },
      {
        item_id: `p_${userId}_interest_hint`,
        content: `[个性化] 该生偏好视觉型学习 + 游戏开发兴趣，推荐使用 Minecraft 风格类比解释编程概念，多用动画/图表辅助理解`,
        category: 'methodology', layer: 'personal', difficulty: 'beginner',
        tags: ['学习风格', '兴趣', '个性化'], metadata: { userId }, relevance_score: 0.85,
      },
    ];
  }
   ...
   * 根据关键词匹配度排序
   */
  private rankByRelevance(results: KnowledgeSearchResult[], query: string): KnowledgeSearchResult[] {
    ...
    return results
      .map((item) => {
        ...
        return { ...item, relevance_score: Math.min(1, matchRatio + layerBonus + item.relevance_score * 0.3) };
      })
      .sort((a, b) => b.relevance_score - a.relevance_score);
  }

  private createMockSearchResults(query: string, layerFilter?: KnowledgeLayer, stage?: LearningStage, topK: number = 5): KnowledgeSearchResult[] {
  ...
  private createMockRAGResult(query: string, stage?: LearningStage, topK: number = 3): RAGResult {
    ...
    const mergedContext = [
      ...
      stageResults.length > 0 ? `[${stage || 'L2'}阶段知识库] ${stageResults.map((r) => r.content).join('；')}` : '',
      personalResults.length > 0 ? `[个人知识库] ${personalResults.map((r) => r.content).join('；')}` : '',
    ].filter(Boolean).join('\n');
    ...
    return {
      ...
      total_score: allItems.reduce((acc, r) => acc + r.relevance_score, 0) / Math.max(allItems.length, 1),
    };
  }
