    ...
    <div className="container mx-auto px-4 py-16">
      ...
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {features.map((feature, index) => (
          ...
          >
            <div className={`absolute top-0 right-0 -mt-4 -mr-4 h-24 w-24 rounded-full ${feature.color} opacity-10 transition-all group-hover:scale-150`} />
            <div className="relative z-10">
              <div className={`mb-4 inline-flex h-12 w-12 items-center justify-center rounded-xl ${feature.color} bg-opacity-10 text-${feature.color.split('-')[1]}-600`}>
                <feature.icon className={`h-6 w-6 text-${feature.color.split('-')[1]}-600`} />
              </div>
              ...
              {(feature.id === 'metaverse-lab' || feature.id === 'gamification') && (
                <span className={`text-sm font-medium flex items-center group-hover:translate-x-1 transition-transform text-${feature.color.split('-')[1]}-600`}>
