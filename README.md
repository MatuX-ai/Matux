...
**完整在线测验系统，含防作弊机制** - 已全部完成 ✅

#### 功能概览
- ✅ 随机抽题考试（计时器 + 题目导航）
- ✅ 4 种题型：单选 / 多选 / 判断 / 填空
- ✅ 自动评分 + 成绩分析
- ✅ 前端防作弊：屏幕切换检测（blur/focus 事件上报）
- ✅ 剪贴板禁用
- ✅ 后端防作弊：答题时间异常分析 + IP/设备聚类检测
- ✅ 考试心跳上报（`/api/v1/exams/{id}/heartbeat`）

#### API 端点
```python
GET    /api/v1/exams                     # 获取可用测验列表
POST   /api/v1/exams/{id}/start          # 开始测验
POST   /api/v1/exams/{id}/submit         # 提交答案
POST   /api/v1/exams/{id}/heartbeat      # 防作弊心跳
GET    /api/v1/exams/{id}/results        # 获取成绩
```

📁 **核心文件**:
- `backend/routes/exam_routes.py` - 测验 API
- `backend/models/exam.py` - Exam/Question/ExamAttempt/CheatEvent 模型
- `backend/services/exam_service.py` - 随机抽题/自动评分
- `backend/services/anti_cheat_service.py` - 防作弊引擎
- `src/app/exam/exam.module.ts` - Angular 懒加载模块
- `src/app/exam/pages/exam-list/` - 测验列表页
- `src/app/exam/pages/exam-taking/` - 答题页面（计时+导航）
- `src/app/exam/pages/exam-result/` - 成绩结果页
- `src/app/exam/services/exam.service.ts` - 前端 API 调用
- `src/app/exam/services/anti-cheat.service.ts` - 前端防作弊

---

### 📴 离线模式增强 (T5)

**IndexedDB v3 大容量离线存储 + 智能同步** - 已全部完成 ✅

#### 数据库设计（IndexedDB v3）
| 对象存储 | 用途 | 索引 |
|---------|------|------|
| `courses` | 课程内容 + 元数据 | courseId, downloadedAt, expiresAt |
| `progress` | 学习进度 | userId/courseId/synced/updatedAt |
| `assets` | Blob 资源（音频/图片） | resourceUrl, mimeType, expiresAt |
| `sync_queue` | 离线操作队列 | (内置 IndexedDB 自动管理) |

#### 冲突解决策略
- **local_wins**: 本地版本优先（乐观策略）
- **remote_wins**: 远程版本优先（保守策略）
- **merged**: 字段级合并（取 completedLessons 最大值，lessonProgress 并集）

#### Service Worker 缓存策略（ngsw-config.json）
| 数据组 | 策略 | 缓存时间 |
|--------|------|---------|
| `api-offline` | freshness | 7 天 |
| `api-performance` | performance | 较大缓存 |
| `static-content` | performance | 30 天 |

#### 同步引擎特性
- ✅ 优先级队列（progress/exam_attempt=高，courses=中，others=低）
- ✅ 批量提交（可配置 batchSize）
- ✅ 指数退避重试（1s → 2s → 4s → 8s → max 30s）
- ✅ 网络质量感知（有效带宽阈值）
- ✅ 同步历史记录（LocalStorage 保留最近 20 条）

📁 **核心文件**:
- `src/app/core/services/offline-storage.service.ts` - IndexedDB v3 封装
- `src/app/offline-mode/services/offline-course-storage.service.ts` - 课程离线缓存
- `src/app/offline-mode/services/offline-progress-storage.service.ts` - 进度同步
- `src/app/offline-mode/services/offline-sync.service.ts` - 批量同步引擎
- `src/app/offline-mode/components/offline-dashboard/` - 离线仪表板
- `ngsw-config.json` - Service Worker 缓存配置

---

### 🌐 Vircadia 元宇宙集成 (T6)

**Angular 3D 虚拟学习环境** - 已全部完成 ✅

#### 模块结构（懒加载）
```
vircadia/
├── vircadia.module.ts          # 路由配置（3 个子路由）
├── vircadia.component.ts       # 主容器（工具栏导航 + router-outlet）
└── pages/
    ├── virtual-classroom/      # 虚拟教室（3 种场景切换）
    ├── virtual-lab/            # 虚拟实验室（5 学科选项卡）
    └── avatar-customization/   # Avatar 换装（body/outfit/accessory）
```

#### 虚拟教室场景
- 标准教室、 lecture hall、讨论室（VircadiaSceneViewerComponent 嵌入）
- `sdk.switchScene()` 场景切换

#### 虚拟实验室
- 5 学科实验选项卡：物理 / 光学 / 电路 / 化学 / 工程
- 实验内容区域（placeholder 预留）

#### Avatar 换装
- 3 类：Body（4种体型）/ Outfit（4种服装）/ Accessory（4种配饰）
- 网格选择面板，视觉反馈
...
```
- `vircadia-server`: 3D 场景服务（端口 9000/9001，2GB 内存限制）
- `vircadia-assets`: 静态资源（Nginx，端口 9010）

📁 **核心文件**:
- `src/app/vircadia/` - Angular 模块（5 个文件）
- `docker-compose.vircadia.yml` - 生产级容器配置

---

### 📱 Flutter 移动端完善 (T7)

**Flutter 核心学习功能完善** - 已全部完成 ✅

#### 新增页面
| 页面 | 功能 |
|------|------|
| `exam_list_screen.dart` | 测验列表（卡片布局 + 难度徽章 + 下拉刷新） |
| `exam_screen.dart` | 答题页（4题型 + 计时器 + 题目圆点导航 + 提交确认） |
| `offline_mode_screen.dart` | 离线管理（网络状态 + 缓存课程 + 存储用量 + 同步管理） |

#### 底部导航扩展
```
底部 Tab: 课程 | 测验 | 进度 | 离线 | 我的
               ↑新增   ↑新增   ↑新增
```

#### 新增依赖
- `firebase_messaging` + `flutter_local_notifications` - 推送通知
- `connectivity_plus` - 网络状态检测

📁 **核心文件**:
- `flutter_app/lib/screens/exam_list_screen.dart` (149行) - 测验列表
- `flutter_app/lib/screens/exam_screen.dart` (273行) - 答题页面
- `flutter_app/lib/screens/offline_mode_screen.dart` (122行) - 离线模式
- `flutter_app/lib/screens/home_screen.dart` - 5Tab 底部导航
- `flutter_app/lib/services/course_service.dart` - 5 个测验 API 方法
- `flutter_app/pubspec.yaml` - 新增推送/网络依赖
...
📁 **AI模型热更新（后端）**:
- 模型管理器: `backend/ai_service/model_manager.py` (621行) - 注册/加载/卸载/AB测试/资源监控
- 模型注册中心: `backend/ai_service/model_registry.py` (267行) - 元数据持久化（JSON/Redis/DB）
- API路由: `backend/ai_service/model_routes.py` (232行) - 9个 RESTful 端点
...
POST   /api/v1/models/{id}/predict      # 模型推理
POST   /api/v1/models/ab-test           # A/B 测试
GET    /api/v1/models/stats/inference   # 推理统计
GET    /api/v1/models/stats/resources   # 资源使用
POST   /api/v1/models/cleanup           # 清理闲置模型
```

**支持的模型类型**: TensorFlow, ONNX Runtime, PyTorch (TorchScript), 自定义 Python 模块
...
- ✅ **桌面端**: Electron 桌面客户端开发中
- ✅ **多模态激励**: 语音/AR/手势三维联动系统上线
- ✅ **AI 个性化教师**: 学习画像 + 上下文记忆 + 成长轨迹追踪
- ✅ **虚拟实验室**: 256 个 3D 电子元件模型库建成
- ✅ **防作弊测验系统**: 随机抽题/计时/自动评分/屏幕切换检测/剪贴板禁用
- ✅ **离线模式增强**: IndexedDB v3 大容量存储 + Service Worker 缓存策略 + 冲突解决
- ✅ **Vircadia 元宇宙集成**: Angular 3D 虚拟教室/实验室/Avatar 换装
- ✅ **Flutter 移动端完善**: 测验答题页/离线模式页/底部5Tab导航
- ✅ **AI 模型热更新**: 后端动态加载/卸载 TensorFlow/ONNX/PyTorch 模型 + A/B 测试
...
- 🏆 **游戏化**: 成就系统 + 积分排行榜 + 推荐算法
- 🏆 **防作弊测验**: 随机抽题 + 心跳监测 + 时间异常分析 + 设备聚类
- 🏆 **离线增强**: IndexedDB v3 (courses/progress/assets) + 批量同步 + 指数退避
- 🏆 **Vircadia 元宇宙**: Angular 懒加载模块 + 虚拟教室/实验室/Avatar
- 🏆 **Flutter 完善**: 测验答题(4题型) + 离线管理 + 底部导航5Tab
- 🏆 **AI 模型热更新**: 后端 ModelManager + ModelRegistry + A/B 测试 + RESTful API
...
│  (学生学习端)    │     │  (课件资源平台)   │     │  (机构管理端)     │
│                 │     │                  │     │                  │
│ • 课程学习      │────→│ • 教程/课件获取   │     │ • 机构/校区管理   │
│ • AI 编程       │     │ • 知识图谱路径   │     │ • 教师/排课管理   │
│ • 虚拟实验      │     │ • 硬件项目资源   │     │ • 设备/Token计费  │
│ • 成长追踪      │     │ • 学习路径推荐   │     │ • 学员管理       │
│                 │     │                  │     │                  │
│     学生        │     │    开发者/教师     │     │   机构管理员      │
└────────┬────────┘     └──────────────────┘     └──────────────────┘
...
> **注意**: 课件管理功能在 OpenMTSciEd 中开发，机构管理功能在 OpenMTEduInst 中开发，MatuX 不再重复开发这些功能，通过 API 调用对应服务。
...
- **构建工具**: Angular CLI
- **桌面端**: Electron 28
- **离线存储**: IndexedDB v3（原生 API）
- **PWA**: Service Worker（多种缓存策略：freshness/performance）
- **3D 渲染**: Vircadia Web SDK（Three.js 场景管理）

### 后端技术栈
- **框架**: FastAPI (Python)
- **数据库**: SQLAlchemy ORM + PostgreSQL / SQLite (开发)
- **缓存**: Redis
- **认证**: JWT + OAuth2
...
- **推荐算法**: 混合推荐（5 维度加权）
- **游戏化**: 成就系统 + 积分排行榜
- **AI 个性化教师**: 学习画像 + 上下文记忆 + 成长轨迹
- **单元测试**: pytest + in-memory SQLite
- **代码质量**: Black 格式化 + 类型注解 95%+
- **模型管理**: ModelManager + ModelRegistry（JSON/Redis/DB 存储）

### 外部项目集成
- **OpenMTSciEd API**: 课件/教程/知识图谱/硬件项目资源 (localhost:3000)
- **OpenMTEduInst API**: 机构/教师/排课/设备管理 (独立部署)
- **互联互通**: 共享用户认证（JWT），学生账号跨项目使用

### 核心功能模块（MatuX 自有）
1. **AI代码生成服务** - 支持多种AI模型的代码辅助生成
2. **智能推荐系统** - 基于用户行为的个性化内容推荐
3. **电商支付系统** - 完整的在线支付和订单管理（Token计费）
4. **硬件认证服务** - 物联网设备接入和认证管理
5. **AI创意激发引擎** - 多维度创意生成、评分和商业价值评估
6. **协作编辑系统** - 基于OT算法的实时协同编辑平台
7. **XR远程教学系统** - 集成AR手势识别、VR代码编辑和白板协作的综合教学平台
8. **课程版本控制系统** - Git-like的课程内容版本管理
9. **AR/VR课程集成** - 沉浸式增强现实和虚拟现实教学体验
10. **区块链积分平台** - 基于Hyperledger Fabric的积分认证系统
11. **多模态激励联动系统** - 语音/AR/手势与积分系统的深度集成
12. **AI 模型热更新系统** - 基于 BLE 的边缘 AI 模型在线升级
13. **OpenHydra 集成平台** - Kubernetes 实训环境深度整合
14. **AI 个性化教师** - 学习画像+上下文记忆+成长轨迹+智能诊断
15. **联动任务系统** - 软硬结合综合实践（95% 准确率）
16. **错误日志收集** - 批量上报与统计分析
17. **赞助活动管理** - 完整的活动创建与追踪系统
18. **Avatar 管理** - Vircadia 虚拟化身验证与优化
19. **防作弊测验系统** - 随机抽题/自动评分/计时考试/防作弊检测（心跳监测+屏幕切换+设备聚类+剪贴板禁用）
20. **离线模式增强** - IndexedDB v3 大容量存储（courses/progress/assets）+ Service Worker 缓存策略 + 冲突解决（local_wins/remote_wins/merged）+ 指数退避重试
21. **Vircadia 元宇宙集成** - Angular 懒加载模块 + 3D 虚拟教室/虚拟实验室/Avatar 换装
22. **Flutter 移动端完善** - 测验答题（4种题型）+ 离线管理 + 底部5Tab导航
23. **AI 模型热更新（后端）** - 动态加载/卸载 TensorFlow/ONNX/PyTorch 模型 + A/B 测试 + 资源监控

### 已解耦至独立项目的模块

| 模块 | 目标项目 | 说明 |
|------|---------|------|
| **课件管理** | [OpenMTSciEd](G:\OpenMTSciEd) | 教程/课件/知识图谱/硬件项目资源 |
| **机构管理** | [OpenMTEduInst](G:\OpenMTEduInst) | 机构/校区/教师/排课/设备/Token计费 |
| **许可证管理** | [OpenMTEduInst](G:\OpenMTEduInst) | 软件授权方案 |
| **多租户管理** | [OpenMTEduInst](G:\OpenMTEduInst) | SaaS多租户权限控制 |
| **权限管理（机构侧）** | [OpenMTEduInst](G:\OpenMTEduInst) | 机构管理员/教师/学校管理员权限 |

> **避免重复开发**: 以上模块的功能请在对应项目中开发，MatuX 通过 API 调用这些服务。
...
### 🔬 STEM 专注
- 虚拟实验室（电路仿真/AR/数字孪生）
- 硬件实践（ESP32 TinyML/BLE 热更新）
- 3D 电子元件模型库

### 📱 全平台适配
- 桌面端（Electron，编程+实验主力）
- 移动端（Flutter，课程学习+日常练习）
- 离线模式支持

## 快速开始

> ⚠️ **避免重复开发**: 课件管理功能在 **OpenMTSciEd** (localhost:3000/api/v1) 中开发，机构管理功能在 **OpenMTEduInst** 中开发，不在 MatuX 中重复。MatuX 通过 API 调用这两个项目的服务。
